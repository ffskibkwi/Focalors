...
int main(int argc, char* argv[])
{
    ...
    physics_config->Ha = case_param.Ha;
    physics_config->Bx = case_param.Bx;
    physics_config->By = case_param.By;
    ...
    MHDModule2D mhd_module(&u, &v, physics_config, time_config, env_config);
    ...
    for (int step = 1; step <= time_config->num_iterations; step++)
    {
        ...
        if (step % pv_output_step == 0)
        {
            ...
            IO::var_to_csv_full(phi, nowtime_dir + "/phi/phi_" + std::to_string(step));
        }
    ...
    IO::var_to_csv_full(phi, nowtime_dir + "/final/phi_" + std::to_string(final_step_to_save));
    return 0;
}
