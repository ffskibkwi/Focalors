...
/**
 * Flow Past Cylinder - Two Domain Validation (MirrorPoint)
 *
 * This case tests the MirrorPoint IBM solver with a cylinder placed exactly on the boundary
...
class FlowPastCylinderTwoDomainMirrorPointCase : public CaseBase
{
public:
    FlowPastCylinderTwoDomainMirrorPointCase(int argc, char* argv[])
        : CaseBase(argc, argv)
    {}
...
{
    FlowPastCylinderTwoDomainMirrorPointCase case_param(argc, argv);
    ...
    std::cout << "=== Flow Past Cylinder - Two Domain (MirrorPoint) ===\n";
    ...
    // MirrorPoint IBM setup - create circle shape
    Circle circle(case_param.cylinder_center_x, case_param.cylinder_center_y, case_param.cylinder_radius);

    // Create MirrorPoint solvers for u and v (no-slip: BC = 0)
    IBSolver2D_MirrorPoint solver_u(&u, PDEBoundaryType::Dirichlet, 0.0);
    solver_u.add_shape(&circle);
    solver_u.build();

    IBSolver2D_MirrorPoint solver_v(&v, PDEBoundaryType::Dirichlet, 0.0);
    solver_v.add_shape(&circle);
    solver_v.build();

    std::cout << "MirrorPoint interior points:\n";
    std::cout << "    u solver: " << solver_u.get_num_interior_points(&d_left) << " (left), "
              << solver_u.get_num_interior_points(&d_right) << " (right)\n";
    std::cout << "    v solver: " << solver_v.get_num_interior_points(&d_left) << " (left), "
              << solver_v.get_num_interior_points(&d_right) << " (right)\n\n";
    ...
    for (int step = 1; step <= case_param.max_step; step++)
    {
        ...
        // Apply MirrorPoint IBM (both u and v solvers)
        solver_u.apply();
        solver_v.apply();
