...
static void compare_velocity_fields_phys(const Variable3D& u_single,
                                         ...
                                         double            rel_eps)
{
    std::cout << "[MirrorPoint 3D] Comparing velocities between single-domain and two-domain setups (by physical coords)...\n";
...
int main(int /*argc*/, char* /*argv*/[])
{
    ...
    std::cout << "MirrorPoint sphere: center=(" << cx << "," << cy << "," << cz << "), radius=" << r << "\n";
    ...
    IBSolver3D_MirrorPoint solver_u_single(&u_single, PDEBoundaryType::Dirichlet, 0.0);
    solver_u_single.add_shape(&sphere);
    solver_u_single.build();

    // 为 v 创建 solver（v 在 y-face 上）
    IBSolver3D_MirrorPoint solver_v_single(&v_single, PDEBoundaryType::Dirichlet, 0.0);
    solver_v_single.add_shape(&sphere);
    solver_v_single.build();

    // 为 w 创建 solver（w 在 z-face 上）
    IBSolver3D_MirrorPoint solver_w_single(&w_single, PDEBoundaryType::Dirichlet, 0.0);
    solver_w_single.add_shape(&sphere);
    solver_w_single.build();
    ...
    // 为 u 创建 solver
    IBSolver3D_MirrorPoint solver_u_multi(&u_multi, PDEBoundaryType::Dirichlet, 0.0);
    solver_u_multi.add_shape(&sphere);
    solver_u_multi.build();

    // 为 v 创建 solver
    IBSolver3D_MirrorPoint solver_v_multi(&v_multi, PDEBoundaryType::Dirichlet, 0.0);
    solver_v_multi.add_shape(&sphere);
    solver_v_multi.build();

    // 为 w 创建 solver
    IBSolver3D_MirrorPoint solver_w_multi(&w_multi, PDEBoundaryType::Dirichlet, 0.0);
    solver_w_multi.add_shape(&sphere);
    solver_w_multi.build();
    ...
    vtk_single.validate();
    vtk_single.write("vtk/ibm_MirrorPoint_3d_single_domain");

    // Case 2: Multi domain
    VTKWriter vtk_multi;
    vtk_multi.add_vector_as_cell_data(&u_multi, &v_multi, &w_multi, "velocity");
    vtk_multi.validate();
    vtk_multi.write("vtk/ibm_MirrorPoint_3d_multi_domain");

    std::cout << "VTK files written to vtk/ibm_MirrorPoint_3d_single_domain.vtu and vtk/ibm_MirrorPoint_3d_multi_domain.vtu\n";

    std::cout << "[MirrorPoint 3D] Validation finished.\n";
